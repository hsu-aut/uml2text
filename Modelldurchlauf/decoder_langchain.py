...
tailOfInputPath = "/generated_text_files/unibw/TwoWay"
...
def read_txt_files(directory):
    uml_files = glob.glob(os.path.join(directory, "*.txt"))
    if not uml_files:
...
def save_description(description, output_directory, filename):
    ...
    output_file = os.path.join(output_directory, os.path.splitext(filename)[0] + ".xmi")
    with open(output_file, "w", encoding="utf-8") as file:
        file.write(description)

def read_system_message():
    prompt_file_path = os.path.join(dirname, "query_scripts", "prompts", "decoder_prompt_system_message.txt")
    with open(prompt_file_path, "r", encoding="utf-8") as file:
        system_message = file.read()
    return system_message

def read_task_description():
    prompt_file_path = os.path.join(dirname, "query_scripts", "prompts", "decoder_prompt_task_description.txt")
    with open(prompt_file_path, "r", encoding="utf-8") as file:
        task_description = file.read()
    return task_description

def main():
    model_data = read_txt_files(inputDir)
